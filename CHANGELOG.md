...
- Fixed the polygon tool doing nothing when clicked. `Scientia Trace` was written as a subclass of `ScientiaPolygonMeasureOperator` to share the point-placing interaction. Blender binds an RNA struct to its Python class through attributes a subclass inherits, so registering an operator and a subclass of it left the parent's struct without a usable class: Blender logged `unable to get Python class for RNA struct 'WM_OT_scientia_polygon_measure'`, the tool stayed in the toolbar and could be selected, and clicking ran nothing. The shared behaviour moved into a mixin that is not an operator and is never registered, so the two tools are independent as far as registration is concerned.

- Fixed an operator surviving an in-place update as an RNA struct with no Python class behind it. Blender then logged `unable to get Python class for RNA struct 'WM_OT_scientia_polygon_measure'`, the tool stayed in the toolbar, its key-map still fired, and clicking did nothing at all because there was nothing to run. Re-running `__init__.py` on an update resets the `classes` record that `unregister` would have cleaned up with, so registration now drops any registration Blender still holds under the same RNA identifier before installing the new class. Looking it up by identifier rather than by class identity is what finds one whose Python class is already gone.

- Fixed a tool being left in the toolbar with nothing bound to it, so selecting it and clicking did nothing at all. Blender names a tool key-map after the tool's label, which makes the name identical across two installed copies of the add-on. The toolbar purge deleted the key-map along with the entry, and with a copy installed both as an extension and as a legacy add-on it took away the key-map the other, live registration was using. The purge now removes the toolbar entry only. A leftover key-map costs nothing, because `register_tool` reuses one of the same name and Blender ignores items whose operator is missing, while `bpy.utils.unregister_tool` still removes the key-map of the entry it owns on the normal path.
