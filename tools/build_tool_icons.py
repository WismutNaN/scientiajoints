...
def _rasterize(triangles, size, background):
    """Scanline-fill the triangles into an RGB pixel buffer, origin top left."""
    pixels = [[list(background) for _ in range(size)] for _ in range(size)]
    for points, color in triangles:
        ...
        for py in y_range:
            for px in x_range:
                ...
                if u < 0.0 or v < 0.0 or u + v > 1.0:
                    continue
                pixels[size - 1 - py][px] = list(color[:3])
    return pixels


def write_preview(path, size=64, background=(58, 58, 58)):
    """Render every icon side by side into a PNG, for eyeballing changes."""
    strips = [_rasterize(builder(), size, background) for _, builder in sorted(ICONS.items())]
    width = size * len(strips)
    rows = [
        b"\x00" + bytes(
            value
            for strip in strips
    ...
    header = struct.pack(">IIBBBBB", width, size, 8, 2, 0, 0, 0)
    png = (b"\x89PNG\r\n\x1a\n"
           + chunk(b"IHDR", header)
